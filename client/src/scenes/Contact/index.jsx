...
import React, { useState, useRef } from "react";
import "./Contact.css"; // Your CSS styles here

export default function Contact({
  emergencyNumbers = {
    police: "100",      // Indian Police emergency number
    ambulance: "108",   // Indian Ambulance emergency number
    fire: "101",        // Indian Fire emergency number
  },
  sosEndpoint = "/api/sos",
}) {
  const [loading, setLoading] = useState(false);
  ...
  // Promise wrapper to get geolocation
  const getCurrentPosition = (options = { enableHighAccuracy: true, timeout: 10000, maximumAge: 0 }) =>
    new Promise((resolve, reject) => {
      if (!("geolocation" in navigator)) {
        reject(new Error("Geolocation not supported."));
  ...
  // Send location payload to backend API
  const sendLocationToAuthorities = async (position) => {
    setLoading(true);
    setMessage(null);
    try {
  ...
  // Open confirmation modal on SOS click
  const handleSOS = () => {
    setModalOpen(true);
  };

  // Confirm SOS: get location and send
  const confirmSOS = async () => {
    setModalOpen(false);
    setLoading(true);
    setMessage(null);
  ...
  // Copy Google Maps location URL to clipboard
  const copyLocationToClipboard = async () => {
    try {
      const pos = await getCurrentPosition();
      const url = `https://maps.google.com/?q=${pos.coords.latitude},${pos.coords.longitude}`;
  ...
  return (
    <div className="contact-page">
      <div className="contact-container">
        <header className="contact-header">
          <h1>Emergency Contacts & SOS</h1>
          <p>Quick access to emergency services and location sharing. Use responsibly in real emergencies only.</p>
        </header>
        ...
        <section className="contact-section">
          <div className="contact-grid">
            ...
            <div className="sos-section">
              ...
              <p className="hint">Sends your location to authorities — use only in emergencies.</p>
            </div>
          </div>
        ...
        <footer className="contact-footer">
          <p>
            This page is for emergency use only. Misuse may result in legal consequences. Location data is shared securely with authorities. Replace placeholder numbers and integrate a secure backend. Privacy: We do not store your location beyond emergency response.
          </p>
        </footer>
      </div>
      ...
      {modalOpen && (
        <div className="modal-overlay">
          <div className="modal">
            <h3>Confirm SOS</h3>
            <p>Are you sure you want to send your location to emergency services? This should only be used in real emergencies.</p>
            <div className="modal-actions">
              <button className="cancel-btn" onClick={() => setModalOpen(false)}>
                Cancel
              </button>
            ...
            <p className="modal-tip">Tip: Call emergency numbers directly for immediate help.</p>
          </div>
        </div>
      )}
    </div>
